({
  fireFilterEvent: function(component,eventName) {    
    ...
    let currencyValue = component.get('v.currencyRate').value;
    if(maxPrice){
      maxPrice *= currencyValue;
    }
    const catigory = component.find("categoryPicklist").get("v.value");
  ...
  formParametersString : function(component){
    ...
    let str ='name=' + name + '&category=' + catigory + '&travel='+distance + '&price=' + maxPrice/currency.value + '&currency=' + currency.value+'&currencyLabel=' + currency.label;
    return str;
  },
